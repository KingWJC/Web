这篇文章就是参考W3C的`XMLHttpRequest`标准和结合一些实践验证总结而来的。

## 1. `Ajax`和`XMLHttpRequest`

我们通常将`Ajax`等同于`XMLHttpRequest`，但细究起来它们两个是属于不同维度的2个概念。

> 以下是我认为对`Ajax`较为准确的解释：（摘自[what is Ajax](https://link.segmentfault.com/?enc=gI2t93i4SZMeZVJ9KIXM2g%3D%3D.HwVh9UWuhFrd%2Bc44LCnGtgDkZvW0Thbfgn4v%2BlFSOintnLsMiI4z8mCw7lznTERzURYPt11wVwwYOEUBnwAF9Q%3D%3D)）
...
`XMLHttpRequest`一开始只是微软浏览器提供的一个接口，后来各大浏览器纷纷效仿也提供了这个接口，再后来W3C对它进行了标准化，提出了[`XMLHttpRequest`标准](https://link.segmentfault.com/?enc=say4jZhAH7QyE5gadMKljQ%3D%3D.4NV0L41SKrXywY2%2BZNmgYwJwoHgNtj7KIMVoAGSPlmWTsOy%2B3EWd6YEyJcXD68Nm)。`XMLHttpRequest`标准又分为`Level 1`和`Level 2`。
...
当然更详细的对比介绍，可以参考[阮老师的这篇文章](https://link.segmentfault.com/?enc=67EtQWUrVJFCAPNn1RKpXw%3D%3D.pLIoPYrpbyEK%2Fbl40bhTsn1cO79CQFj5kCz4BCPxXeZIKv6P3tcCSzirOz%2BiUhfJeE8dty5l2SWQ0HPZBQ2lfk30PcKNRdboaJoPG3gUErM%3D)，文章中对新增的功能都有具体代码示例。

## 3. `XMLHttpRequest`兼容性

关于`xhr`的浏览器兼容性，大家可以直接查看“Can I use”这个网站提供的结果[XMLHttpRequest兼容性](https://link.segmentfault.com/?enc=y4Aq2VBJn8pji7ZEVUt1bw%3D%3D.5%2BfynefUtt%2Bc66CthlNPGqc17qDjsfv73yAhwaFg8YDIiv0EeSyTtcJH8K3oiK2u)，下面提供一个截图。

![clipboard.png](images/bVsiDk.png)
...
### 4.1 如何设置request header

在发送`Ajax`请求（实质是一个[HTTP](https://link.segmentfault.com/?enc=RnC0bOXP5Ck8mvWItnGFbg%3D%3D.cEaAhXSdF2q%2FCNCjb%2B5boJ6fHIZtdfwm5Plm5eeJtAi12FvXPMft%2FzgR5OaJ8kCpHsSRvAAuSPgYL09ELFlzLw%3D%3D)请求）时，我们可能需要设置一些请求头部信息，比如`content-type`、`connection`、`cookie`、`accept-xxx`等。`xhr`提供了`setRequestHeader`来允许我们修改请求 header。
...
经过一番寻找最终在 [Stack Overflow找到了答案](https://link.segmentfault.com/?enc=xqrM4Q%2Brukqy%2FQ8FtR3yjA%3D%3D.BkAJrhuwZ%2Fs%2FOhc9N9LK8IiRmomLauj0I4QzbIx3oOY03dNo01NKUhS6JeBcxaC15obzWpVhU20Pdy9C6T4ppAk81RVHDqNBNdHD9xAFHWCdETUd%2F%2FilosdWgCUN9xAL)。

- 原因1：[W3C的 xhr 标准中做了限制](https://link.segmentfault.com/?enc=s0Tibi%2BTw0%2BWiUiOiT1SBg%3D%3D.lNNT9MvtP85344E77OVUG15Bmky%2BTkPIaCfszyp3Kt8t5wQZzHDaeJCE8RNquUge)，规定客户端无法获取 response 中的 `Set-Cookie`、`Set-Cookie2`这2个字段，无论是同域还是跨域请求；
- 原因2：[W3C 的 cors 标准对于跨域请求也做了限制](https://link.segmentfault.com/?enc=xNow9T4bGofwVFJDG%2FVP6Q%3D%3D.gensirUC94rdpWWTz0VMN46VwU0fJXWMMWYPANFrWByVjWnONRKLZjWw%2FnmdLyGwiRCstIbCXypNwGfT%2Fae4L%2FDqRHXLiBfLyWcBvLJcxmc%3D)，规定对于跨域请求，客户端允许获取的response header字段只限于“`simple response header`”和“`Access-Control-Expose-Headers`” （两个名词的解释见下方）。
...
### 4.10 `xhr.withCredentials`与 `CORS` 什么关系
...
#### 5.3.1 发生`abort`/`timeout`/`error`异常的处理
...
- 想真正搞懂`XMLHttpRequest`，最靠谱的方法还是看 [W3C的xhr 标准](https://link.segmentfault.com/?enc=396SpW9RtSXt5uFzOrVXTg%3D%3D.16C%2FWxTHaHJeQaQP%2FhgqgK1fRsBJ1J%2FyIMiXFB%2FTJ0CxkOT92pG7GZp0v5LqscFe);
- 想结合代码学习如何用`XMLHttpRequest`发各种类型的数据，可以参考[html5rocks上的这篇文章](https://link.segmentfault.com/?enc=2ZS8A%2BnBatbcPkClm0RHNg%3D%3D.vM5wqk2z3leM1JBvElF70TeM%2FfyDgsfdzzIbrFo8IiJUJItmFUedwRVqJzH2HH2fpG3aMfhw0g2ss7I%2F3c01iA%3D%3D)
- 想粗略的了解`XMLHttpRequest`的基本使用，可以参考[MDN的XMLHttpRequest介绍](https://link.segmentfault.com/?enc=b5ExdubIPJvioZ3ocJ2Dxw%3D%3D.nbCF5SUxuw31AtiKsfxCZnjDX%2FapnHLWxLtE04kA%2BHKrUjNelTgJPIWbIqgGC5B8Eoy62%2B4%2BWBvdUjwvi2CX6A%3D%3D)；
- 想了解`XMLHttpRequest` 的发展历程，可以参考[阮老师的文章](https://link.segmentfault.com/?enc=0Zc6ZFCaNovxlIHyijAo%2BQ%3D%3D.uSoqqNXuZJc3henIDtJDDsEClPznBexzfNsop1GIk44L7qC8mSVPSCA0UOJAcNgKz0vAIsncolAiYYtxc34Jmk4qypSTLP6nm5ES1emJvqI%3D)；
- 想了解`Ajax`的基本介绍，可以参考[AJAX Tutorial](https://link.segmentfault.com/?enc=s38Zb7%2BXVyRLqUCBKd%2BHUA%3D%3D.QYKNSoopUyOqu%2FQqwFsXQ%2BgJJ6GKE1hjAj2CuEAq47PQ1yzslXNfzkASP49qT2jv);
- 想了解跨域请求，则可以参考[W3C的 cors 标准](https://link.segmentfault.com/?enc=IiWMwmDmqMrG%2BixgU%2BoFRw%3D%3D.BimeL%2FqyJzKb2k3vUmlJGge07tMD6F1LcWkTdepx2aY%3D);
- 想了解`http`协议，则可以参考[HTTP Tutorial](https://link.segmentfault.com/?enc=xmmsTPsJ583WLhQljfi1qw%3D%3D.3RVtxEVra8edcowPyzBgFv5i2fMj7Y9MwTd9neA%2BpzclLScu5bZfGxbo6N5jKzB4Fm4%2FkqF5%2B47jxeUH1A0N4Q%3D%3D);